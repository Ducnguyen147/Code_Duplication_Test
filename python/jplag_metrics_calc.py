...
- Precision, Recall, F1 using "same-prefix" (e.g., C03_*) as gold.
"""
...
def group_prefix(path: str, depth: int = 1) -> Optional[str]:
    ...
    Example: '/.../C03_gcd_type1_a.py' -> 'C03' (depth=1)
             depth=2 -> 'C03_gcd'
    Returns None if no underscore is present or if path is empty.
    """
...
def parse_csv_text(text: str) -> List[Dict[str, str]]:
    ...
    """
    # Try to sniff a dialect; fall back to comma.
    sample = text[:4096]
    try:
...
def parse_pretty_table_text(text: str) -> List[Dict[str, str]]:
    ...
    for line in text.splitlines():
        ...
        # Split left/right by at least two spaces to be safe
        parts = re.split(r"\s{2,}", rest)
        if len(parts) < 2:
            # last resort: split once on whitespace
...
def evaluate(rows: List[Dict[str, str]], threshold: float, group_depth: int, sort_rows: bool):
    # Map headers
...
def main():
    ...
    ap.add_argument(
        ...
        help="Underscore-based prefix depth used as 'group' (1 -> C03, 2 -> C03_gcd). Default: 1"
    )
    ap.add_argument(
    ...
    # Load and evaluate
    rows = load_rows(text)
    res = evaluate(rows, threshold=args.threshold, group_depth=args.group_depth, sort_rows=args.sort)
