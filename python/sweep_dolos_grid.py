...
Notes
-----
- Requires `dolos` on PATH. See Dolos CLI docs for flags -k/-w and default output to terminal. 
- We run Dolos once per (k,w) and reuse its stdout for all thresholds to save time.
"""
...
def run_filter_on_stdout(filter_script: str, threshold_pct: int, dolos_stdout: str) -> Tuple[Optional[int], Optional[int], Optional[int], Optional[float], Optional[float], Optional[float], str]:
    """
    Pipe `dolos_stdout` into `python dolos_stdout_filter.py -t {threshold}%`
...
def main():
    ...
    for k in grid_k:
        for w in grid_w:
            ...
            for t in thresholds:
                tp, fp, fn, precision, recall, f1, raw = run_filter_on_stdout(args.filter_script, t, dolos_out)
                if tp is None:
