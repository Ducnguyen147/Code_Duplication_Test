...
Adds Dolos/MOSS-style token fingerprinting (k-grams with optional winnowing) and uses
coverage-style gates (fp_sim / fp_total / fp_longest) to suppress false positives on tiny files.
...
def kgram_hashes(tokens: List[str], k: int) -> List[int]:
    ...
    for i in range(len(tokens) - k + 1):
        ...
        # rolling hash not necessary here; sha1 is fine for small corpora
...
def winnow(hashes: List[int], w: int) -> List[Tuple[int,int]]:
    ...
    for start in range(0, len(hashes) - w + 1):
        ...
        if pos != last_pos:  # avoid duplicates when same min slides
            fps.append((m, pos))
            last_pos = pos
    return fps
...
def longest_common_run(seqA: List[int], seqB: List[int]) -> int:
    ...
    if not seqA or not seqB:
        return 0
    # DP for longest common substring (O(N*M); fine for tiny files)
    ...
    for path in file_paths:
        ...
        if use_lex:
            if lex_mode == "py-token" and toks:
                ...
                # default fallback: character n-grams over raw text
                s = text if text else ""
                if not s or len(s) < lex_n:
                    lex_rows.append(np.zeros(lex_dim, np.float32))
                else:
                    # simple char n-gram hashing (stable)
...
def build_faiss_index_np(mat: np.ndarray) -> faiss.Index:
    ...
    dim = mat.shape[1]
    index = faiss.IndexFlatIP(dim)  # inner product; L2-normalized -> cosine
...
def main() -> None:
    ...
    print(f"Channels active → embed:yes  ast:{'yes' if ast_active else 'no'}  lex:{'yes' if lex_active else 'no'}")
    ...
    if args.debug_components and args.fusion == "late":
        for p1, p2, sim in out_pairs:
            ...
            # FP metrics
            fpS = jaccard_from_counters(fp_ctr[i], fp_ctr[j]) if use_fp else 0.0
            fpT = total_overlap(fp_ctr[i], fp_ctr[j]) if use_fp else 0
            fpL = longest_common_run(fp_seq[i], fp_seq[j]) if use_fp else 0
