#!/usr/bin/env python3
"""
Sweep detect_clone_cli_v8.py over --min-tokens and thresholds; write TP/FP/FN/Precision/Recall/F1 to CSV.
...
- Ignores "unclassified" pairs (where either file doesn't match the regex) in FP/TN counts,
  and computes FN against all gold-positive file pairs.

CSV schema
----------
language_label,min_tokens,threshold_pct,tp,fp,fn,precision,recall,f1,raw_pairs_txt

Example
-------
python sweep_detect_v8_grid.py \
  --script ./detect_clone_cli_v8.py \
  --dir /abs/path/to/mix \
  --extensions .py \
  --mode hybrid \
...
Notes
-----
- If your dataset is large, consider lowering --prefilter-topM or omitting --mutual-nearest
  to trade quality vs. speed.
"""
...
PAIR_RE = re.compile(r"^\s*(?P<sim>\d+(?:\.\d+)?)\t(?P<left>[^\t]+)\t(?P<right>[^\t]+)")
...
def run_detector(
    ...
    *,
    ...
    """
    Call detect_clone_cli_v8.py and capture stdout.
...
def parse_pairs(stdout_text: str) -> List[Tuple[str, str, float]]:
    ...
    Ignore any other lines (headers, debug, etc).
    """
    ...
    for line in stdout_text.splitlines():
        ...
        if left and right and left != right:
            # Normalize unordered pair key by sorting later, metrics keep original strings
...
def build_gold_pairs(files: List[str], pat: re.Pattern) -> Tuple[Dict[str, List[str]], List[Tuple[str, str]]]:
    ...
    for g, lst in groups.items():
        if len(lst) >= 2:
            # generate all unordered combinations
            L = sorted(lst)
...
def evaluate_threshold(
    ...
    pred_set = set(tuple(sorted((a,b))) for (a,b) in pred)
    ...
    for a,b in pred_set:
        ...
        if ga is None or gb is None:
            # ignore unclassified predicted pairs in FP/TN accounting (consistent with your other tools)
            continue
        if ga == gb:
            tp += 1
        else:
...
def main():
    ap = argparse.ArgumentParser(description="Sweep detect_clone_cli_v8.py (min-tokens, thresholds) → CSV metrics.")
    ap.add_argument("--script", default="./detect_clone_cli_v8.py", help="Path to detect_clone_cli_v8.py")
    ap.add_argument("--dir", required=True, help="Root directory of submissions")
    ap.add_argument("--extensions", nargs="+", required=True, help="File extensions to include (e.g., .py .java)")
    ap.add_argument("--mode", default="hybrid", choices=["hybrid","semantic","semantic-plus","structural"], help="Detector mode")
    ...
    ap.add_argument("--extra-args", nargs="*", default=[], help="Any extra args to pass through to detector (e.g. --no-strip-comments)")
    ...
    ap.add_argument("--group-regex", default=r"^(C\d+)(?:[_-]|(?=[A-Z]))",
                    help=r"Regex with ONE capturing group to define 'group' from basename (default handles C06_*, C06Wordcount*)")
    ...
    # Build full file list once for gold (must mirror detector's extension filtering).
    ...
    for mt in min_tokens_list:
        ...
        print(f"[+] detect_clone_cli_v8.py  min_tokens={mt}  mode={args.mode}", file=sys.stderr)
        ...
        # Persist raw pairs for inspection/repro
        try:
