...
def fp_similarity(i: int, j: int, seq: List[List[int]], ctr: List[Counter], mode: str = "combo") -> float:
    if mode == "jaccard":
        return jaccard_from_counters(ctr[i], ctr[j])
    elif mode == "overlap":
        tot = sum(ctr[i].values()) + sum(ctr[j].values())
        return (2.0 * total_overlap(ctr[i], ctr[j]) / tot) if tot > 0 else 0.0
    elif mode == "longest":
...
# --------------------------- Structural pipeline (AST + lex + FP) --------------
    ...
    *,
    ...
    for path in file_paths:
        ...
        if p_lower.endswith(".py"):
            ...
            toks = ts_token_stream(text, parser, lang_name or "lang") if parser is not None else c_like_fallback_token_stream(text)
        else:
...
def all_pairs_neighbors(n: int) -> List[List[int]]:
...
def compute_pairs_late(
    ...
    for i in range(n):
        ...
        for j in neigh_idx[i]:
            ...
            if fp_active:
                sim_fp = fp_similarity(i, j, fp_seq, fp_ctr, mode=fp_mode_for_score)
                num += w_fp * sim_fp; den += w_fp
...
# --------------------------- CLI main ------------------------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Hybrid semantic+structural clone detection with (k,w) fingerprints in-score")
    ...
    ap.add_argument("--mode", choices=["hybrid", "semantic", "semantic-plus", "structural"], default="hybrid",
                    help="Scoring mode")
    ap.add_argument("--min-tokens", type=int, default=5, help="Sensitivity knob; used if --fp-k unset")
    ...
    ap.add_argument("--no-fp-in-score", dest="use_fp_in_score", action="store_false",
                    help="Disable fingerprint similarity from the final score")
    ap.set_defaults(use_fp_in_score=True)

    ap.add_argument("--fp-weight", type=float, default=0.35, help="Weight for fingerprint similarity in final score")
    ap.add_argument("--fp-sim-mode", choices=["jaccard","overlap","longest","combo"], default="combo",
                    help="Fingerprint similarity used in the score")
    ...
    ap.add_argument("--prefilter", choices=["embed","none"], default="embed",
                    help="Neighbor prefilter: semantic (embed) or none (all pairs; slower)")
    ...
    ap.add_argument("--threshold", type=float, default=None, help="Similarity threshold; sweep script will set 0.0")
    ...
    ap.add_argument("--model", default="BAAI/bge-code-v1", help="SentenceTransformer model")
    args = ap.parse_args()
    ...
    if faiss is None and args.prefilter == "embed":
        print("Error: requires 'faiss-cpu' for semantic prefilter.", file=sys.stderr); sys.exit(1)

    _auto_profile(args, N)
    print(f"[AUTO] mode={args.mode}, min_tokens={args.min_tokens} → "
          f"k={args.fp_k}, w={args.fp_w}, min_fp_sim={args.min_fp_sim:.3f}, "
    ...
    if use_center:
        ...
        centered = l2_normalize_rows(embed_np_raw - mu)
    ...
    w_fp = float(args.w_fp)  if args.use_fp_in_score else 0.0
    ...
    print(f"Channels → embed:{'yes' if w_e>0 else 'no'}  ast:{'yes' if ast_active else 'no'}  lex:{'yes' if lex_active else 'no'}  fp:{'yes' if w_fp>0 else 'no'}")
    print(f"Weights  → w_e={w_e}  w_ast={w_a}  w_lex={w_l}  w_fp={w_fp}")
    ...
    if args.prefilter == "embed":
        ...
        neigh_idx = all_pairs_neighbors(N)

    pairs = compute_pairs_late(
        files, embed_for_final, ast_np, lex_np,
        w_embed=w_e, w_ast=w_a, w_lex=w_l, w_fp=w_fp,
        fp_mode_for_score=args.fp_sim_mode,
