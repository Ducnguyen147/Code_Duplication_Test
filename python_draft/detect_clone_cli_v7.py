#!/usr/bin/env python3
"""
Hybrid semantic + structural (AST / lexical) clone detection with FAISS + fingerprint gating (v7.1).

Key fixes vs v7:
- Disable mean-centering for tiny cohorts (N<20) or when it collapses vectors (near-zero norms).
- Use raw (L2-normalized) embeddings for prefilter; optionally center for final stage only if safe.
- Slightly lower default final thresholds for 'hybrid' (0.32) and ease more for very tiny N (–0.02).
- Keep STRICT fingerprint gate for short files; gate uses MOSS-style winnowing.

Users only need: --dir --extensions --mode --min-tokens (+ optional --topk --debug-components)
        ...
        for k in range(start, end):
            ...
            if token_counts is not None and (token_counts[i] < short_token_gate or token_counts[j] < short_token_gate):
                if not (fpS >= min_fp_sim and fpT >= min_fp_total and fpL >= min_fp_longest):
                    # allow superpass only if embedding is extremely high
                    if sim_e < embed_superpass:
                        continue

            # structural gate (so we don't accept pure semantic with no structural signal unless superpass)
            gate_fail = True
...
def _auto_profile(args, N: int):
    ...
    # --- thresholds ---
    # prefilter: a low bar to collect candidates
    ...
    # Tiny cohorts: do NOT raise thresholds; slightly ease them.
    if N < 8:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Hybrid semantic+structural clone detection (FAISS) + fingerprint gate (v7.1)")
    ...
    if use_center:
        ...
        # If any row collapsed (norm≈0), skip centering
    ...
    print(f"Prefilter: cosine≥{args.prefilter_threshold} | Final threshold={args.threshold} | Short-token gate={20} tokens")
