#!/usr/bin/env python3
"""
Hybrid semantic + structural (AST / lexical) clone detection with FAISS + fingerprint gating (v7.2).

Key changes vs v7.1:
- Evidence guard: in HYBRID, require SOME structural (AST/lex) or fingerprint signal for a pair,
  unless the embedding "superpass" is extremely high. This removes semantically-hubby false positives.
- Tiny cohorts (N<10): stricter defaults (higher prefilter/final thresholds, higher superpass).
- AST stop-features: drop the top-K most frequent AST buckets (default 64) before TF-IDF.
- Optional mutual-kNN filter: keep a pair only if i∈topK(j) and j∈topK(i) in the embedding space.

Background:
  - Winnowing local fingerprinting (MOSS) — Schleimer et al., SIGMOD’03.
  - Dolos-style overlap metrics (total overlap, longest fragment).
  - FAISS cosine via L2-normalized dot product; safe mean-centering for the final stage.

Usage (simple):
  python3 detect_clone_cli_v7_2.py --dir /path/to/src --extensions .py --mode hybrid --min-tokens 5

Flags you might tweak for precision:
  --mutual-knn 5                 # enable mutual-kNN pruning
  --debug-components             # prints per-pair (e/ast/lex/fp) components

python3 detect_clone_cli_v6.py --dir /home/duc/Desktop/code_duplication/Code_Duplication_Test/python/obfuscation --extensions .py --min-tokens 5 --mode hybrid --mutual-knn 5 --debug-components
python3 detect_clone_cli_v6.py --dir /home/duc/Desktop/code_duplication/Code_Duplication_Test/python/type1 --extensions .py --
min-tokens 5 --mode hybrid --mutual-knn 5 --debug-components --model BAAI
/bge-code-v1
"""
...
def embed_files(
    ...
    """
    Encode each file as the mean of chunk embeddings; return L2-normalized rows.
    """
...
def compute_topk_neighbors(base: np.ndarray, k: int) -> List[Set[int]]:
    """
    For each row i, return the set of its top-k neighbor indices (excluding i).
    """
    if k <= 0:
        return []
    index = build_faiss_index_np(base)
    D, I = index.search(base, k + 1)
    N = base.shape[0]
    neigh: List[Set[int]] = []
    for i in range(N):
        s: Set[int] = set()
        for j in I[i]:
            ...
            if jj >= 0 and jj != i:
                s.add(jj)
        # limit to k neighbors if more were returned
        if len(s) > k:
            s = set(list(s)[:k])
        neigh.append(s)
    return neigh
...
def compute_pairs_late(
    ...
    for i in range(n):
        ...
        for k in range(start, end):
            ...
            # Optional mutual-kNN pruning
            if mutual_k > 0 and mutual_neighbors is not None:
                if not (j in mutual_neighbors[i] and i in mutual_neighbors[j]):
                    continue
            ...
            # --- global evidence guard (NEW): require some structural OR fingerprint signal, unless superpass
            fp_ok = (fpS >= min_fp_sim) or (fpT >= min_fp_total) or (fpL >= min_fp_longest)
            struct_ok = ((ast_active and sim_a >= min_ast_sim) or
                         (lex_active and sim_l >= min_lex_sim))
            if not (fp_ok or struct_ok) and sim_e < embed_superpass:
                continue

            # If we *do* have structure, blend it in
...
def _auto_profile(args, N: int):
    ...
    args.lex_dim  = 4096
    args.ast_tfidf = True
    args.ast_stop_topk = 64     # NEW: drop frequent AST features
    ...
    if args.mode == "semantic":
        ...
        args.threshold = 0.32
        # NEW: demand a little structural evidence even in hybrid
        args.min_ast_sim = 0.025
        args.min_lex_sim = 0.025

    # Tiny cohorts: be conservative (avoid FPs)
    if N < 10:
        args.prefilter_threshold = 0.30
        args.threshold = max(args.threshold, 0.36)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Hybrid semantic+structural clone detection (FAISS) + fingerprint gate (v7.2)")
    ...
    ap.add_argument("--model", default="mchochlov/codebert-base-cd-ft",
                    help="SentenceTransformer model (e.g., jinaai/jina-embeddings-v2-base-code, BAAI/bge-code-v1)")
    ...
    ap.add_argument("--mutual-knn", type=int, default=0,
                    help="Keep pair only if mutual top-K neighbors in embedding space (0=disabled)")
    args = ap.parse_args()
    ...
    print(f"[AUTO] mode={args.mode}, min_tokens={args.min_tokens} → "
          ...
          f"ast_stop_topk={args.ast_stop_topk}, mutual_knn={args.mutual_knn}")
    ...
    if use_center:
        ...
        # If any row collapses (norm≈0), skip centering
    ...
    ast_np, lex_np, tokens_by_file, fp_seq, fp_ctr, tok_counts = compute_structural_features(
        files,
        ast_dim=2048, use_ast=use_ast, ast_tfidf=True, ast_stop_topk=args.ast_stop_topk,
    ...
    print(f"Prefilter: cosine≥{args.prefilter_threshold} | Final threshold={args.threshold} | "
          f"Short-token gate={20} tokens")

    # Optional: mutual-kNN neighbor sets
    mutual_neighbors = None
    if args.mutual_knn and args.mutual_knn > 0:
        mutual_neighbors = compute_topk_neighbors(prefilter_np, int(args.mutual_knn))
    ...
    pairs = compute_pairs_late(
        ...
        mutual_k=int(args.mutual_knn or 0),
    )
    ...
    if args.debug_components:
        ...
        for p1, p2, sim in out_pairs:
            ...
            print(f"{sim:.4f}\t{p1}\t{p2}\t(e={sim_e:.4f}, ast={sim_a:.4f}, lex={sim_l:.4f}, "
                  f"fp_sim={fpS:.3f}, fp_tot={fpT}, fp_long={fpL})")
    else:
        for p1, p2, sim in out_pairs:
