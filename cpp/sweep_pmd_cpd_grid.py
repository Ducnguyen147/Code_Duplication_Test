...
TP/FP/FN/Precision/Recall/F1 into a CSV (Dolos-style), with optional --group-regex.

- min_tokens: 5,10,25,40,60  (override with --min-tokens)
- thresholds: 50..95 step 5  (override with --thresholds)
- output CSV: language,min_tokens,threshold_pct,group_regex,tp,fp,fn,precision,recall,f1

Notes:
- CPD is invoked via the PMD launcher: `pmd cpd ...`.
- We add `--no-fail-on-violation` and `--no-fail-on-error` so CPD exits 0 but still writes the report. [docs]
"""
...
def main():
    ...
    # Include group_regex for reproducibility
