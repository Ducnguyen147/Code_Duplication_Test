...
It expects your metrics script:
  jplag_metrics_calc.py  (supports: -t <percent>% -i <results.csv> [--sort] [--group-regex REGEX])

Examples
--------
# Python-style names (underscore prefixes)
...
# Java-style CamelCase prefixes (forward regex to metrics)
python sweep_jplag_grid.py \
  --jar jplag-6.0.0-jar-with-dependencies.jar \
  --input mix \
  --language java \
  ...
  --out final-jplag/mix_java_metrics.csv

Notes
-----
- JPlag CLI flags: -l (language), -t (min tokens), -r (result base),
  --csv-export (write results.csv), --mode run (don’t launch viewer).
- We create a distinct result base per t (e.g., result-jplag/mix_t3),
  so each run yields its own results.csv.
- This sweeper expects the metrics script to print lines like:
...
def run_jplag(jar: str, language: str, input_root: str, t_value: int, result_base: str) -> Tuple[int, str, str, str]:
    ...
    # By convention, CSV lands under <result_base>/results.csv
